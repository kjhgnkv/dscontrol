...
#include <QtDesigner/private/iconloader_p.h>
#include <QtDesigner/private/previewmanager_p.h>
#include <QtDesigner/private/codedialog_p.h>
...
QDesignerActions::QDesignerActions(QDesignerWorkbench *workbench)
    : QObject(workbench),
      ...
      m_addCustomWidgetAction(new QAction(tr("Add custom widget"), this)),
      ...
      m_appFontAction(new QAction(tr("Additional Fonts..."), this))
{
    ...
    m_addCustomWidgetAction->setObjectName(QStringLiteral("__qt_add_custom_widget_action"));
    ...
    m_addCustomWidgetAction->setProperty(QDesignerActions::defaultToolbarPropertyName, true);
...
//
    ...
    connect(m_addCustomWidgetAction, &QAction::triggered, this, static_cast<VoidSlot>(&QDesignerActions::addCustomWidget));
    m_fileActions->addAction(m_addCustomWidgetAction);
...
void QDesignerActions::addCustomWidget()
{
    AddCustomWidgetDialog dialog;
    dialog.exec();
}
...
#else
    if (!qdesigner_internal::CodeDialog::showCodeDialog(fw, qdesigner_internal::UicLanguage::Cpp, fw, &errorMessage))
#endif
