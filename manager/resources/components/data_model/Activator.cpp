...
//clean up model registration
if(
modelHub &&hubRef
) {
modelHub->unregModel("{{model_id}}");//unregister
if(model) {
delete
model;//delete model
model = NULL;
...
if(pCxt) {
pCxt->
ungetService(hubRef);//unget service
}
}
hubRef = 0;
modelHub = 0;//clear model hub
...
Activator::getServices()
{
    ...
    if (pCxt && (!hubRef))
    {
        QList<ctkServiceReference> refs = pCxt->getServiceReferences(QString("dm::DataModelHub"),
                                                                     QString());//get references of service
        foreach(const ctkServiceReference &ref, refs)
            {
                modelHub = dynamic_cast<dm::DataModelHub*>(pCxt->getService(ref));//try get service
                if (modelHub)
                {
                    hubRef = ref;//record reference
                    break;
                }
            }
    }
...
Activator::regServices()
{
    ...
    if (modelHub && hubRef)
    {
        modelHub->regModel(model);//register model
    }
}
