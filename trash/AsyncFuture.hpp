...
#define ASYNCFUTURE_ERROR_OBSERVE_VOID_WITH_ARGUMENT                           \
    "Observe a QFuture<void> but your callback contains an input argument"
#define ASYNCFUTURE_ERROR_CALLBACK_NO_MORE_ONE_ARGUMENT                        \
    "Callback function should not take more than 1 argument"
#define ASYNCFUTURE_ERROR_ARGUMENT_MISMATCHED                                  \
    "The callback function is not callable. The input argument doesn't match " \
    "with the observing QFuture type"

#define ASYNC_FUTURE_CALLBACK_STATIC_ASSERT(Tag, Completed)                    \
    static_assert(Private::arg_count<Completed>::value <= 1,                   \
                  Tag ASYNCFUTURE_ERROR_CALLBACK_NO_MORE_ONE_ARGUMENT);        \
    static_assert(!(std::is_same<void, T>::value &&                            \
                    Private::arg_count<Completed>::value >= 1),                \
                  Tag ASYNCFUTURE_ERROR_OBSERVE_VOID_WITH_ARGUMENT);           \
    static_assert(                                                             \
        Private::decay_is_same<T, Private::Arg0Type<Completed>>::value ||      \
            Private::arg0_is_future<Completed>::value ||                       \
            std::is_same<void, Private::Arg0Type<Completed>>::value,           \
        Tag ASYNCFUTURE_ERROR_ARGUMENT_MISMATCHED);
    ...
    // we specialize for pointers to member function
{
    ...
    // If the result_type is a QFuture<T>, the type will be T. Otherwise, it is
    // void
...
/* It is an additional to the original function_traits to handle non-const
 * function (with mutable keyword lambda). */
...
struct decay_is_same : std::is_same<T, typename std::decay<U>::type>::type
{
};
...
template <typename Functor> using RetType = typename function_traits<Functor>::result_type;

template <typename Functor> using Arg0Type = typename arg0_traits<Functor>::type;
...
struct is_callable
{
    enum
    {
        value = (arg_count<Functor>::value == 1) && (!std::is_same<void, T>::value) && (std::is_convertible<Arg0Type<Functor>, T>::value)
    };
};
...
// AsyncFuture does not use QVariant because it needs user to register before
// use.
template <typename R>
class Value
{
...
void watch(QFuture<T> future, QObject* owner, QObject* contextObject, Finished finished, Canceled canceled)
{
    Q_ASSERT(owner);
    ...
    if (owner)
    {
        // Don't set parent as the context object as it may live in different
        // thread
        QObject::connect(owner, &QObject::destroyed, watcher, &QObject::deleteLater);
    }

    if (contextObject)
    {
        QObject::connect(watcher, &QFutureWatcher<T>::finished, contextObject, [=]()
        {
            delete watcher;
        ...
        QObject::connect(watcher, &QFutureWatcher<T>::canceled, contextObject, [=]()
        {
            delete watcher;
            if (ownerAlive.isNull())
            {
    ...
    if ((QThread::currentThread() != QCoreApplication::instance()->thread()) && (contextObject == 0 || QThread::currentThread() != contextObject->thread()))
    {
...
/* DeferredFuture implements a QFutureInterface that could complete/cancel a
 * QFuture.
 *
 * 1) It is a private class that won't export to public
 *
 * 2) Its member function do not use <T> to avoid to use template specialization
 * to handle <void>. Type checking should be done by user classes (e.g Deferred)
 *
 */
...
public:
    ...
    void complete(QFuture<T> future)
    {
        ...
        watch(future, this, nullptr, onFinished, onCanceled);
    ...
    void complete(QFuture<QFuture<ANY>> future)
    {
        ...
        watch(future, this, 0, onFinished, onCanceled);
    ...
    {
        incWeakRefCount();
        QObject::connect(sender, member, this, [=]()
        {
            this->cancel();
            decWeakRefCount();
        });
...
protected:
    DeferredFuture(QObject* parent = nullptr)
    : QObject(parent)
...
private:
    // A reference count system. If it is dropped to zero, it will cancel this
    // object
    int refCount;
    ...
    template <typename ANY>
    typename std::enable_if<!std::is_same<ANY, void>::value, void>::type completeByFinishedFuture(QFuture<T> future)
    {
...
{
public:
    CombinedFuture(bool settleAllModeArg = false)
    : DeferredFuture<void>()
    ...
    static QSharedPointer<CombinedFuture> create(bool settleAllMode)
    {
        ...
        {
            // Regardless of the no. of instance of
            // QSharedPointer<CombinedFuture>, it only increase the reference by
            // one.
            object->decStrongRef();
            object->decWeakRefCount();
        };
...
template <typename Functor, typename T>
typename std::enable_if<is_callable<Functor, T>::value, void>::type callIgnoreReturn(Functor &functor,
                                                                                     QFuture<T> &future)
{
    functor(future);
}
...
template <typename Functor, typename T>
typename std::enable_if<is_callable<Functor, T>::value, RetType<Functor>>::type call(Functor &functor,
                                                                                     QFuture<T> &future)
{
...
/* eval() : Evaluate the expression - "return functor(future)" that may have a
 * void return type */

template <typename Functor, typename T>
typename std::enable_if<ret_type_is_void<Functor>::value && arg_count_is_zero<Functor>::value, Value<RetType<Functor>>>::type eval(
Functor functor, QFuture<T> future)
{
    Q_UNUSED(future);
    functor();
    return Value<void>();
...
Functor functor, QFuture<T> future)
{
    // callIgnoreReturn() is designed to reduce the no. of annoying compiler
    // error messages.
...
/// Create a DeferredFuture that will execute the callback functions when
/// observed future finished
...
static QFuture<DeferredType> execute(QFuture<T> future, QObject* contextObject, Completed onCompleted,
                                     Canceled onCanceled)
{
    auto defer = DeferredFuture<DeferredType>::create();

    watch(future, contextObject, contextObject, [=]()
    {
        try
        {
...
public:
    ...
    typename std::enable_if<!Private::future_traits<typename Private::function_traits<Completed>::result_type>::is_future, Observable<typename Private::function_traits<Completed>::result_type>>::type context(
    QObject* contextObject, Completed functor)
    {
        /* functor return non-QFuture type */

        ASYNC_FUTURE_CALLBACK_STATIC_ASSERT("context(callback): ", Completed);

        return _context<typename Private::function_traits<Completed>::result_type, typename Private::function_traits<Completed>::result_type>(
        contextObject, functor);
    }

    template <typename Completed>
    typename std::enable_if<Private::future_traits<typename Private::function_traits<Completed>::result_type>::is_future, Observable<typename Private::future_traits<typename Private::function_traits<Completed>::result_type>::arg_type>>::type context(
    QObject* contextObject, Completed functor)
    {
        ...
        return _context<typename Private::future_traits<typename Private::function_traits<Completed>::result_type>::arg_type, typename Private::function_traits<Completed>::result_type>(
        contextObject, functor);
    }
    ...
    typename std::enable_if<!Private::ret_type_is_future<Completed>::value, Observable<typename Private::RetType<Completed>>>::type subscribe(
    Completed onCompleted, Canceled onCanceled)
    {
        /* For functor return a regular value */

        ASYNC_FUTURE_CALLBACK_STATIC_ASSERT("subscribe(callback): ", Completed);

        return _subscribe<typename Private::function_traits<Completed>::result_type, typename Private::function_traits<Completed>::result_type>(
    ...
    template <typename Completed, typename Canceled>
    typename std::enable_if<Private::ret_type_is_future<Completed>::value, Observable<typename Private::function_traits<Completed>::future_arg_type>>::type subscribe(
    Completed onCompleted, Canceled onCanceled)
    {
        ...
        return _subscribe<typename Private::future_traits<typename Private::function_traits<Completed>::result_type>::arg_type, typename Private::function_traits<Completed>::result_type>(
        onCompleted, onCanceled);
    }
    ...
    template <typename Functor>
    typename std::enable_if<std::is_same<typename Private::RetType<Functor>, void>::value, void>::type onProgress(
    Functor functor)
    {
        onProgress([=]() mutable
                   {
    ...
    template <typename Functor>
    typename std::enable_if<std::is_same<typename Private::RetType<Functor>, bool>::value, void>::type onProgress(
    Functor onProgressArg)
    {
        QFutureWatcher<T>* watcher = new QFutureWatcher<T>();
...
private:
    ...
    {
        auto future = Private::execute<ObservableType, RetType>(m_future, contextObject, functor, []()
        {
        });
    ...
    {
        auto future = Private::execute<ObservableType, RetType>(m_future, QCoreApplication::instance(), onCompleted,
                                                                onCanceled);
...
public:
    inline Combinator(CombinatorMode mode = FailFast)
    : Observable<void>()
    {
...
template <typename Member>
auto observe(QObject* object,
             Member pointToMemberFunction)->Observable<typename Private::signal_traits<Member>::result_type>
{
